...
function buildPublishableKey(frontendApi) {
	const isDevKey = PUBLISHABLE_FRONTEND_API_DEV_REGEX.test(frontendApi) || frontendApi.startsWith("clerk.") && require_constants.LEGACY_DEV_INSTANCE_SUFFIXES.some((s) => frontendApi.endsWith(s));
	const keyPrefix = isDevKey ? PUBLISHABLE_KEY_TEST_PREFIX : PUBLISHABLE_KEY_LIVE_PREFIX;
	return `${keyPrefix}${require_isomorphicBtoa.isomorphicBtoa(`${frontendApi}$`)}`;
}
/**
...
function isPublishableKey(key = "") {
	try {
		...
		const decoded = require_isomorphicAtob.isomorphicAtob(encodedPart);
...
async function getCookieSuffix(publishableKey, subtle = globalThis.crypto.subtle) {
	...
	return require_isomorphicBtoa.isomorphicBtoa(stringDigest).replace(/\+/gi, "-").replace(/\//gi, "_").substring(0, 8);
}
/**
...
//# sourceMappingURL=keys-CHdYWeEj.js.map