...
const HotelReg = () => {
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            const {data} = await axios.post(`/api/hotels/register`,{name, contact, address, city},{headers: {Authorization:`Bearer ${await getToken()}`}})
            if(data.success){
  ...
  return (
    <div onClick={() => setShowHotelReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
      <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
        <img src={assets.regImage} alt="reg-image" className='w-1/2 roundeed-xl hidden md:block'/>
        <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
            ...
            {/* Addres */}
            <div className='w-full mt-4'>
                <label htmlFor="addres" className='font-medium text-gray-500'>
                    Addres
                </label>
                <input onChange={(e) => setAddress(e.target.value)} value={address} id='addres' type="text" placeholder='Type here' className='border border-gray-200 rounded w-full px-3 py-2.5 mt-1 outline-indigo-500 font-light' required/>
            </div>
